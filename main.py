...
def train(model, supervisor):
	...
	trX, trY, num_tr_batch, valX, valY, num_val_batch = load_data(cfg.batch_size, cfg.length, is_training=True)
...
def evaluation(model, supervisor):
	teX, teY, num_te_batch = load_data(cfg.batch_size, cfg.length, is_training=False)
